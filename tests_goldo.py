...
DAMAGE_RECVD_DEFLECTED = """[23:49:54.399] [Vilus Garr {3054408942223360}:685001717997] [@Lor'dan] [Pistol Shot {3128737646247936}] [ApplyEffect {836045448945477}: Damage {836045448945501}] (0 -deflect {836045448945508}) <1>"""
DAMAGE_RECVD_PARRIED = """[23:47:39.133] [Kell Dragon {3067057620910080}:4632001698367] [@Lor'dan] [Spines {3148979827113984}] [ApplyEffect {836045448945477}: Damage {836045448945501}] (0 -parry {836045448945503}) <1>"""
...
class TestParserFunctions(unittest.TestCase):
    ...
    def test_damage_received_deflected(self):
        file_ = io.StringIO()
        file_.writelines(DAMAGE_RECVD_DEFLECTED)
        file_.seek(0)
        log_file = csv.DictReader(
            file_, fieldnames=main.CSV_HEADER, delimiter=']',
            skipinitialspace=True)
        for row in log_file:
            self.parser.dispatch_row(row)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Vilus Garr "]["Pistol Shot "]["total_damage"], 0)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Vilus Garr "]["Pistol Shot "]["dodged"], 1)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Vilus Garr "]["Pistol Shot "]["hit"], 0)

    def test_damage_received_parried(self):
        file_ = io.StringIO()
        file_.writelines(DAMAGE_RECVD_PARRIED)
        file_.seek(0)
        log_file = csv.DictReader(
            file_, fieldnames=main.CSV_HEADER, delimiter=']',
            skipinitialspace=True)
        for row in log_file:
            self.parser.dispatch_row(row)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Kell Dragon "]["Spines "]["total_damage"], 0)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Kell Dragon "]["Spines "]["dodged"], 1)
        self.assertEqual(self.parser.pull["damage_received"]["Lor'dan"]["attackers"]["Kell Dragon "]["Spines "]["hit"], 0)
